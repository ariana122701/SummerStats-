...
This report presents preliminary data analysis for the Week 3 Example Data. Specifically this will report descriptive statistics as well as report tests of the following predictions: 

1. People will feel more guilt when: 
  + 1.1 They caused the shock (vs. their partner) 
  + 1.2 Their partner was morally good (vs. bad) 
  + 1.3 Their shock was stronger (high > medium > low)
...
data<-read.csv("week3/process_data/Week 3 Data PROCESSED.csv")
...
Descriptives statistics for guilt: 
```{rdescGuilt}
...
Descriptives statistics for generosity: 
```{rdescGenerosity}
...
### Prediction 1.1
Prediction 1.1 was supported. People felt more guilty when they caused the shock then when their partner did. 
```{r p1.1}
...
Prediction 3 was supported. Agreeeable people felt more guilty 
```{r p3}
#Run the t-test for Prediction 3 
...
Predicion was not supported. Neurotic people were not more guilty. 
```{r p4}
